...
# import data
setwd("E:/Network analysis/Network analysis AD/2 Control, AD and MCI seperately/3 MCI")
load("AfterMergedMEs.Rdata")
load("TOMs and geneTree.Rdata")  #to get TOM
setwd("E:/Network analysis/Network Analysis AD/1 Data loaded in")
...
expr.data = MCIdatExpr
n.iterations=400
meg = 0
job.path = "E:/Network analysis/Network analysis AD/4 k-means correction seperate networks/3 MCI"
tom.matrix=TOM
plot.evolution=TRUE
plot.go=FALSE
debug=NULL
net.type="signed"
final.net="E:/Network analysis/Network analysis AD/4 k-means correction seperate networks/3 MCI/All Data.10.signed.it.100.rds"
min.genes.for.grey=20

cat("Working with",nrow(expr.data),"samples and",ncol(expr.data),"genes\n")
##Working with 142 samples and 13661 genes
...
#The network includes 13661 genes and  32 modules
...
#############################################
net.label = "MCI Data genes Exhanged in k-means adjustment"
g.changes = NULL
n = length(partitions)
for(index in 1:(n-1)){
