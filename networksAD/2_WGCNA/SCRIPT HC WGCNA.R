...
sft = pickSoftThreshold(CTLdatExpr, powerVector = powers, verbose = 5, networkType = "signed")
#indicates a softpower of 6 as the best
...
##############################################
#use the soft power of 6 to create network  #
##############################################

softPower = 6 
##create adjacency matrix
adjacency = adjacency(CTLdatExpr, power = softPower, type = "signed")

#######plot to see the scale free topology
k=softConnectivity(datE=CTLdatExpr,power=softPower)
sizeGrWindow(10,5)
par(mfrow=c(1,2))
hist(k)
...
MEList= moduleEigengenes(CTLdatExpr, colors= dynamicColors,softPower = softPower)
...
########################################
MEDissThres = 0.05
merge = mergeCloseModules(CTLdatExpr, dynamicColors, cutHeight = MEDissThres, verbose = 3)
#drops 58 to 58 modules

# merged module colors
moduleColors = merge$colors
names(moduleColors) <- colnames(CTLdatExpr)
#eigengenes of the new merged modules:
mergedMEs = merge$newMEs

# create list of colours starting with grey and then assign colours
colorOrder = c("grey", standardColors(60))
moduleLabels = match(moduleColors, colorOrder)-1
names(moduleLabels) <- colnames(CTLdatExpr)
MEs=mergedMEs
rownames(MEs)<-rownames(CTLdatExpr)
