...
sft = pickSoftThreshold(MCIdatExpr, powerVector = powers, verbose = 5, networkType = "signed")
...
adjacency = adjacency(MCIdatExpr, power = softPower, type = "signed")

#######plot to see the scale free topology
k=softConnectivity(datE=MCIdatExpr,power=softPower)
sizeGrWindow(10,5)
par(mfrow=c(1,2))
hist(k)
...
MEList= moduleEigengenes(MCIdatExpr, colors= dynamicColors,softPower = softPower)
...
########################################
MEDissThres = 0.05
merge = mergeCloseModules(MCIdatExpr, dynamicColors, cutHeight = MEDissThres, verbose = 3)
#drops 35 to 32 modules

# merged module colors
moduleColors = merge$colors
names(moduleColors) <- colnames(MCIdatExpr)
...
names(moduleLabels) <- colnames(MCIdatExpr)
MEs=mergedMEs
rownames(MEs)<-rownames(MCIdatExpr)
