...
sft = pickSoftThreshold(HCdatExpr, powerVector = powers, verbose = 5, networkType = "signed")
#indicates a softpower of 10 as the best
...
##############################################
#use the soft power of 10 to create network  #
##############################################

softPower = 10
##create adjacency matrix
adjacency = adjacency(HCdatExpr, power = softPower, type = "signed")

#######plot to see the scale free topology
k=softConnectivity(datE=HCdatExpr,power=softPower)
sizeGrWindow(10,5)
par(mfrow=c(1,2))
hist(k)
...
MEList= moduleEigengenes(HCdatExpr, colors= dynamicColors,softPower = softPower)
...
########################################
MEDissThres = 0.05
merge = mergeCloseModules(HCdatExpr, dynamicColors, cutHeight = MEDissThres, verbose = 3)
#drops 27 to 25 modules

# merged module colors
moduleColors = merge$colors
names(moduleColors) <- colnames(HCdatExpr)
...
names(moduleLabels) <- colnames(HCdatExpr)
MEs=mergedMEs
rownames(MEs)<-rownames(HCdatExpr)
