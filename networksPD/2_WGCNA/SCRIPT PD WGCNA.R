...
##### import datasets
setwd("/media/jack11/Jack Kelly DRF/Network analysis/Network Analysis PD (newest)/1 Sorting data to be used")
...
powers = c(c(1:12), seq(from = 13, to=20, by=2))
sft = pickSoftThreshold(PDdatExpr, powerVector = powers, verbose = 5, networkType = "signed")
#indicates a softpower of 13 as the best
...
##############################################
#use the soft power of 13 to create network  #
##############################################

softPower = 13
##create adjacency matrix
adjacency = adjacency(PDdatExpr, power = softPower, type = "signed")

#######plot to see the scale free topology
k=softConnectivity(datE=PDdatExpr,power=softPower)
sizeGrWindow(10,5)
par(mfrow=c(1,2))
hist(k)
...
MEList= moduleEigengenes(PDdatExpr, colors= dynamicColors,softPower = softPower)
...
########################################
MEDissThres = 0.05
merge = mergeCloseModules(PDdatExpr, dynamicColors, cutHeight = MEDissThres, verbose = 3)
#drops 61 to 54 modules

# merged module colors
moduleColors = merge$colors
names(moduleColors) <- colnames(PDdatExpr)
...
names(moduleLabels) <- colnames(PDdatExpr)
MEs=mergedMEs
rownames(MEs)<-rownames(PDdatExpr)

save(MEs, moduleLabels, moduleColors , file = "AfterMergedMEs.Rdata")



# calculate the intramodular connectivity to see how well specific genes are connected to other genes within same module
